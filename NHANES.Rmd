...
combine_plot = function(bar, line, indicator){
  title_g = paste("The Total Percent of HIV Positive Patients from 1999 to 2016 by", indicator)
...
combine_mar = combine_plot(bar_sexbehave, line_sexbehave, "Sexual Behavior")
...
line_behavior =
  cleaned_joined_df %>% 
  drop_na(hiv, samesexcontact) %>% 
  mutate(samesexcontact = recode(samesexcontact, "0" = "No same-sex sexual behaviour", 
                                  "1" = "Has same-sex sexual behaviour")) %>% 
  group_by(year, samesexcontact) %>% 
  summarize(age_mean = round(mean(age), 2), 
            hiv_percent = round(sum(hiv == "Reactive") / sum(hiv != "Reactive", hiv == "Reactive") * 100, 3)) %>%
  ungroup() %>% 
