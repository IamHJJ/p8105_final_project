...
Bivariate analysis
```{r}
variables = c("age","gender","race","samesexcontact","marriage")

fit_and_summarize <- function(var) {
  model = lm(as.formula(paste("hiv_outcome~", var)), data = cleaned_regression_df) |>
  broom::tidy()
}
...
```
Variables `age`, `gender`, `race`, `samesexcontact`, `marriage` were found significant associated with HIV infection status, at 5% level of significance. Therefore, these variables were included to the final model.
